	...
	class UserController extends controller {
		...
		public function updateUser($id){
			if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])){
				...
				if(strlen($name) < $Min || strlen(($name)) > $Max){
					$Errors[] = "Name can not be less than {$Min} or more than {$Max} character!";
					$check_validation = false;
				}
				...
				if (!preg_match("/^[0-9]*$/",$phone)) {
					$Errors[] = "Invalid phone numbers format";
					$check_validation = false;
				}
				// if (!preg_match("/^[a-zA-Z,0-9]*$/",$name)) {
				...
				if($check_validation == true){
					$check = json_decode($this->user->update($id, $email, md5($password), $name, $dob, $phone, $file_name, $active));
